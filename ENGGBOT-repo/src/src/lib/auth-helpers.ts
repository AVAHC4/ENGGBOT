...
export function checkExternalAuth(): boolean {
  ...
  if (hasAuthCookie || isAuthenticatedLS || isAuthenticatedSS || hasAuthParam) {
    ...
    if ((hasAuthParam || userName || userEmail || userAvatar) && window.history && window.history.replaceState) {
      // Remove the auth_success parameter from URL without refreshing
      const newUrl = window.location.pathname + 
        window.location.search.replace(/[?&]auth_success=true/, '').replace(/[?&]user_name=[^&]*/, '').replace(/[?&]user_email=[^&]*/, '').replace(/[?&]user_avatar=[^&]*/, '') +
