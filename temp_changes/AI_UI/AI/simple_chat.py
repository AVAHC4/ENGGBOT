#!/usr/bin/env python3
"""
Simple Interactive Chat with DeepSeek-R1 via Chutes AI
...
- aiohttp (for async API calls): pip install aiohttp
- asyncio (for async functionality): pip install asyncio
...
def main():
    ...
    print("\nMain Model: deepseek-ai/DeepSeek-R1-0528")
    ...
    print("Type 'async' to toggle async mode")
    ...
    while True:
        ...
        # Check for other commands
            ...
            print(f"\nAsync mode: {'ENABLED' if async_enabled else 'DISABLED'}")
            if async_enabled:
                print("DeepSeek will now use asynchronous API calls.")
            else:
                print("DeepSeek will now use synchronous API calls.")
            continue
        ...
        # Process normal chat input
            ...
            if async_enabled:
                # Run async version
                asyncio.run(async_chat(client, user_input, streaming_enabled, thinking_enabled))
            else:
...
async def async_chat(client, user_input, streaming_enabled, thinking_enabled):
    """Handle async chat interactions"""
    if not streaming_enabled:
        print("\nDeepSeek is thinking...")
        ...
        response = ""
        async for chunk in client.generate_async(
            prompt=user_input,
            temperature=0.7,
            max_tokens=1024,
            model=client.default_model,
        ...
        # Use invoke_chute for direct streaming
